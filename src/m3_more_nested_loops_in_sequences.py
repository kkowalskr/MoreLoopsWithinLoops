...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 440
    answer = largest_number(([5], [140, 440, 5], [60, 40]))
    print('Expected and actual are:', expected, answer)

    # Test 5:
    expected = 100
    answer = largest_number(([5 , 6, 7], [100, 1], []))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for j in range(len(seq_seq)):
        for k in range(len(seq_seq[j])):
            if seq_seq[j][k - 1] >= seq_seq[j_for_max][k_for_max]:
                k_for_max = k - 1
                j_for_max = j

    if seq_seq[j_for_max] == []:
        return None
...
def run_test_largest_negative_number():
    ...
    # Test 4:
    expected = -5
    answer = largest_negative_number(([5], [140, -440, -5], [-60, 40]))
    print('Expected and actual are:', expected, answer)

    # Test 5:
    expected = -6
    answer = largest_negative_number(([5, -6, 7], [-100, 1], []))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for g in range(len(seq_seq)):
        for k in range(len(seq_seq[g])):
            if seq_seq[g][k] < 0:
                if seq_seq[g][k] > seq_seq[j_for_neg][k_for_neg]:
                    j_for_neg = g
                    k_for_neg = k

    if seq_seq[j_for_neg] == []:
        return None
    if seq_seq[j_for_neg][k_for_neg] >= 0:
        return None
    return seq_seq[j_for_neg][k_for_neg]
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for j in range(len(seq_seq) - 1):
            for g in range(len(seq_seq[j + 1])):
                if len(seq_seq[j + 1]) == 0:
                    break
                if seq_seq[0][k] == seq_seq[j + 1][g]:
                    return True
